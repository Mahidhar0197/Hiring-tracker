...
db.view("hiring_tracker/update",{success: function(data) {
    for (i in data.rows) {
      var temp = data.rows[i].value.hiring_ticket_type;
      var temp1 = data.rows[i].value._id;
       $("#update > tbody").append('<tr id="' + temp1 
							 + '" class="updaterow">'
                             +'<td>'+temp+'</td>'
      ...
      else
      {
        ...
        $(temp1).append('<td>'+data.rows[i].value.hiring_manager    
                             ...
                             +'</td><td><form action="update_position_pop.html">'
